    ...
    const renderComposerContent = () => {
        ...
        return (
            <>
                ...
                <form
                    className="flex flex-col gap-2 rounded-[24px] border border-slate-200/90 bg-white p-2 shadow-lg shadow-slate-900/5 transition focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-500/20 dark:border-white/[0.10] dark:bg-zinc-900 dark:shadow-black/40"
                    onSubmit={(e) => {
                        e.preventDefault()
                        handleSendMessage()
                    }}
                >
                    <textarea
                        ...
                        className="scrollbar-none order-1 max-h-[240px] min-h-[44px] w-full min-w-0 resize-none overflow-y-auto border-0 bg-transparent px-2 py-2.5 text-[max(16px,0.95rem)] leading-relaxed text-slate-900 outline-none placeholder:text-slate-400 dark:text-zinc-100 dark:placeholder:text-zinc-500"
                    />
                    <div className="order-2 flex w-full items-center justify-between gap-2">
