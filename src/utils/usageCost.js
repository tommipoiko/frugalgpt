...
const normalizeUsage = normalizeLegacyUsage
...
const computeOpenAiCost = (pricing, extras, usage) => {
    const {
        inputRate, outputRate, longContextApplied
    } = resolveOpenAiLongContextRates(pricing, usage)
...
const computeMistralCost = (pricing, extras, usage) => {
    ...
    const webSearchUsd = toolRequests * (
        extras.webSearchUsdPerRequest ?? extras.agentToolUsdPerRequest ?? 0.01
    )
...
const computeTurnCost = (provider, modelId, rawUsage, messages, assistantResponse) => {
    ...
    if (!pricing) {
        return {
            usage, costUsd: 0, costSource, costBreakdown: null
        }
    }
